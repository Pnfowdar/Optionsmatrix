...
def load_watchlist() -> list[str]:
    """Loads watchlist from JSON, returns defaults on failure."""
    defaults = cfg.get("TICKERS", ["SPY", "QQQ"])[:] # Get default from config
    ...
    if WATCHFILE.exists():
        try:
            ...
            if isinstance(data, list) and all(isinstance(t, str) for t in data):
                 print(f"DEBUG [load_watchlist] SUCCESS: Loaded {len(data)} tickers.")
                 return data # Return loaded data if valid
            else: # File exists but format is wrong
                 ...
                 save_watchlist(defaults) # Overwrite bad file
                 return defaults
        ...
        save_watchlist(defaults) # Create file with defaults
        return defaults

def save_watchlist(ticker_list: list[str]):
    """Saves cleaned, sorted list to watchlist JSON with error reporting."""
    try:
        ...
        print(f"DEBUG [save_watchlist] SUCCESS: Saved {len(unique_tickers)} tickers to {WATCHFILE}")
    except (IOError, OSError) as e: # Catch file system errors specifically
        st.error(f"CRITICAL ERROR: Could not write to watchlist file '{WATCHFILE}'. Check permissions! Error: {e}")
        print(f"ERROR [save_watchlist] File write failed (IO/OS Error): {e}")
